...
static bool is_logarithm(const std::string& str) {
    std::regex logarithm("^log_[-]?([0-9]+([.][0-9]*)?|[.][0-9]+$)");
    return !str.empty() && (std::regex_match(str, logarithm) || str == "lg" || str == "ln");
}

static bool is_operator(const std::string& str) {
    return (operators_set.find(str) != operators_set.end() || is_logarithm(str));
}
...
void ArithmeticTask::analyze_operator(std::string op) {
    if (is_logarithm(op)) {
        ...
        output.pop();
        if (op == "ln") {
            std::unique_ptr<ICalculatable> result = std::make_unique<LogarithmNatural>(std::move(left));
            output.push(std::move(result));
        } else if (op == "lg") {
            std::unique_ptr<ICalculatable> result = std::make_unique<Logarithm>(std::move(left), 10);
            output.push(std::move(result));
        } else {
            size_t pos = op.find('_');
            if (pos == std::string::npos) {
                throw TaskInvalidData("Wrong format");
            }
            double base = std::stod(op.substr(pos + 1, Expression.size() - pos - 1));
            std::unique_ptr<ICalculatable> result = std::make_unique<Logarithm>(std::move(left), base);
            output.push(std::move(result));
        }
