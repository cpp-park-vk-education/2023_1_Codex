...
void ArithmeticTask::analyze_operator(std::string op) {
    ...
    if (is_binary(op)) {
        ...
        if (op == "+") {
            ...
            std::unique_ptr <ICalculatable> result = std::make_unique<Subtraction>(std::move(left),
                                                                                    std::move(right));
            output.push(std::move(result));
        } else if (op == "/") {
            ...
            std::unique_ptr <ICalculatable> result = std::make_unique<Multiplication>(std::move(left),
                                                                                        std::move(right));
            output.push(std::move(result));
        } else if (op == "^") {
            std::unique_ptr <ICalculatable> result = std::make_unique<Degree>(std::move(left), std::move(right));
            output.push(std::move(result));
        }
...
void ArithmeticTask::ParseData() {
    ...
    std::unique_ptr <ICalculatable> number = std::make_unique<Number>(
            0); //Если первый символ невведен, то как будто там ноль
