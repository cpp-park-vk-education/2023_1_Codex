...
//SolutionWindow::SolutionWindow(Client::Client& client, QString value, QWidget* parent) : m_client(client), typeName(value), QMainWindow(parent), ui(new Ui::SolutionWindow)
SolutionWindow::SolutionWindow(QString value, QWidget* parent) : typeName(value), QMainWindow(parent), ui(new Ui::SolutionWindow){
    ui->setupUi(this);
    //getAnswer("");
    ...
    connect(okButton.get(), &QPushButton::clicked, [=](){
...
void SolutionWindow::setClient(Client::Client& client){
    //m_client = client;
}
...
void SolutionWindow::getAnswer(QString expression){
    ...
    std::cout << "typeName = " << typeName.toStdString() << endl;
    //cout<<m_client.Run(text.toStdString(), taskType).TaskData<<endl;
    //solutionResult = m_client.Run(expression.toStdString(), taskType);
