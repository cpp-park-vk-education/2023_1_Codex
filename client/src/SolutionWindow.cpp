...
SolutionWindow::SolutionWindow(const Client::ClientSPtr& client, Client::TaskTypes taskTypeInp, QString value,
                               QWidget* parent)
    : m_client(client),
      taskType(taskTypeInp),
      typeName(value),
      QMainWindow(parent),
    ...
    if (taskType == Client::Arithmetic) {
        ...
        uploadImageButton->setStyleSheet(
            "background-color: #15E099;color: white;font: 900 14pt \"Century "
            "Gothic\";\nborder-radius:10px;\n");
    ...
    textExpression->setStyleSheet(
        "color: white; font-size: 14px; font: \"Century Gothic\";border-style: solid; border-width: 1px; "
        "border-color: white;");  // убрать бордер
    textExpression->setText("");

    textSolution = std::make_shared<QLabel>(this);
    textSolution->setGeometry(260, 150, this->width() - 260 - 10, 41);
    textSolution->setStyleSheet(
        "color: white; font-size: 14px; font: \"Century Gothic\"; border-style: solid; border-width: 1px; "
        "border-color: white;");  // убрать бордер
    ...
    // для матриц
    if (taskType == Client::MatrixDet || taskType == Client::MatrixT || taskType == Client::MatrixEigen ||
        taskType == Client::MatrixRank || taskType == Client::MatrixNorm) {
    ...
    // интегралы
    else if (taskType == Client::IntegNewton || taskType == Client::IntegSimpson ||
             taskType == Client::IntegTrapez) {
    ...
    // комбинаторика
    else if (taskType == Client::CombComb || taskType == Client::CombRe || taskType == Client::CombPlace ||
             taskType == Client::CombPart) {
    ...
    // polynomial
    else if (taskType == Client::EqPoly) {
        polinomialLabel = std::make_shared<QLabel>("equation:", this);
        polinomialLabel->setStyleSheet("color: white; font-size: 20px; font: \"Century Gothic\"");
        polinomialLabel->setGeometry(130, 320, 100, 40);

        inputField = std::make_shared<QLineEdit>(this);
        inputField->setGeometry(130 + 130, 320, 350, 40);
        inputField->setStyleSheet("color: white; font-size: 18px; font: \"Century Gothic\"");

        constEquationsLabel = std::make_shared<QLabel>(" = 0", this);
        constEquationsLabel->setStyleSheet("color: white; font-size: 20px; font: \"Century Gothic\"");
        constEquationsLabel->setGeometry(620, 320, 80, 40);

        xEquationsLabel = std::make_shared<QLabel>("x<sub>0</sub> = ", this);
        xEquationsLabel->setStyleSheet("color: white; font-size: 20px; font: \"Century Gothic\"");
        xEquationsLabel->setGeometry(130, 370, 80, 40);

        cEquationsinputField = std::make_shared<QLineEdit>(this);
        cEquationsinputField->setGeometry(130 + 130, 370, 100, 40);
        cEquationsinputField->setStyleSheet("color: white; font-size: 20px; font: \"Century Gothic\"");

        iterationsEquationsLabel = std::make_shared<QLabel>("iteartions:", this);
        iterationsEquationsLabel->setStyleSheet("color: white; font-size: 20px; font: \"Century Gothic\"");
        iterationsEquationsLabel->setGeometry(130, 420, 100, 40);

        iterationsEquationsinputField = std::make_shared<QLineEdit>(this);
        iterationsEquationsinputField->setGeometry(130 + 130, 420, 100, 40);
        iterationsEquationsinputField->setStyleSheet(
            "color: white; font-size: 20px; font: \"Century Gothic\"");
    }
    // стандартное
    else {
        inputField = std::make_shared<QLineEdit>(this);
...
void SolutionWindow::getImagePath() {
    QString fileName = QFileDialog::getOpenFileName(
        this, tr("Choose"), "", tr("Images (*.png *.jpg)"), nullptr,
...
QString SolutionWindow::parseIntegral() {
    QString text = "";
    text += inputField->text() + " , " + lowerLimitField->text() + " , " + upperLimitField->text() + " , " +
            numberOfNodesField->text();
    return text;
}
...
QString SolutionWindow::parsePoly() {
    QString text = "";
    text += inputField->text() + " , " + cEquationsinputField->text() + " , " +
            iterationsEquationsinputField->text();
    return text;
}
...
void SolutionWindow::setExpressionSolution() {
    ...
    if (taskType == Client::Arithmetic) {
        inputField->setText(QString::fromStdString(solutionResult.TaskData));
    }
}
